...
const ModifyClassForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                console.log('Class modified:', data);
                setSuccessMessage('Clase modificada correctamente');
                setErrorMessage('');
            } else {
                // Handle errors, e.g., display an error message
                console.error('Failed to modify class');
                setErrorMessage('Error al modificar la clase');
            }
        } catch (error) {
    ...
    return (
        <div className="container mt-5">
            <h1 className="mb-4 text-center">Modificar Clase</h1>
